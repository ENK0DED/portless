...
- #278 is planned as a fork-specific RFC 8441 Extended CONNECT bridge. The plan lives in `docs/superpowers/plans/2026-06-18-upstream-pr-278-h2-websocket.md` and requires exact route matching, loopback-only backend dialing, h2c-route exclusion, internal-host rejection, and `Sec-WebSocket-Accept` validation before payload forwarding.
...
| Upstream PR                                                                                     | Plan                                                                | Notes                                                                                                                                                                                                                                                                      |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [#278 WebSocket-over-HTTP/2 Extended CONNECT](https://github.com/vercel-labs/portless/pull/278) | `docs/superpowers/plans/2026-06-18-upstream-pr-278-h2-websocket.md` | Planned. Add RFC 8441 Extended CONNECT for browser HMR WebSockets over HTTPS HTTP/2 while preserving strict route matching, exact tunnel aliases, path prefixes, multiplexed host selection, internal reserved pages, h2c-route safety, and loopback-only backend dialing. |
...
| [#278 WebSocket-over-HTTP/2 Extended CONNECT](https://github.com/vercel-labs/portless/pull/278) | planned                 | This enables browser HMR WebSockets over the same HTTPS/2 connection already negotiated with portless. Current proxy supports normal HTTP/2 requests and classic HTTP/1.1 WebSocket upgrades, but it does not advertise `SETTINGS_ENABLE_CONNECT_PROTOCOL` or bridge HTTP/2 `CONNECT` streams to backend WebSocket upgrades. The fork plan accepts the feature direction but requires route selection through current fork rules, loopback-only backend dialing, h2c-route rejection, internal-host rejection, exact public tunnel aliases, and `Sec-WebSocket-Accept` validation before forwarding payload bytes. |
...
Rechecked against upstream on 2026-06-18: GitHub reported 48 open PRs, and every open PR is listed below with a fork state. The WebSocket-over-HTTP/2 work is now planned with a fork-specific implementation plan; every other open PR is implemented, implemented differently, or won't implement.
...
| [#278 WebSocket-over-HTTP/2 Extended CONNECT](https://github.com/vercel-labs/portless/pull/278)      | planned                 | See the planned protocol work section and `docs/superpowers/plans/2026-06-18-upstream-pr-278-h2-websocket.md`.                                                                                                                                                                                  |
