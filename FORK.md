...
| [#333 Add background app management](https://github.com/vercel-labs/portless/pull/333)                                  | implemented differently | `33546d8`..`9b1d098` | Added first-party `portless bg` app lifecycle management with locked registry state, private logs, readiness-file handshakes, status/list/logs, graceful stop/restart/clean, and `clean`/`prune` integration. Unlike upstream, the fork does not parse stdout for readiness or kill arbitrary same-port processes. |
