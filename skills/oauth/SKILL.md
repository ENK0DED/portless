...
Bare suffixes like `.dev` mean `myapp.dev` could collide with a real domain. Use a dotted suffix under a domain you control, so the app name stays clean and the domain structure lives in the suffix:

```bash
portless proxy start --suffix local.yourcompany.dev
portless myapp next dev
