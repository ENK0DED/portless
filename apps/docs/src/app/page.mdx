...
The app name is inferred from portless config, `package.json`, git root, or directory name. Use `portless.json` or `.config/portless.json` to override (e.g. `{ "name": "myapp" }` for `https://myapp.localhost`).
...
For other tools, use exact command placeholders. Portless replaces whole arguments matching `{PORT}`, `{HOST}`, or `{PORTLESS_URL}` after assigning the app port and skips automatic framework flag injection when any placeholder is present.
...
By default, portless uses `.localhost` which auto-resolves to `127.0.0.1` in most browsers. Repeat `--suffix` to serve the same apps under multiple suffixes.
Each value may be a single label such as `test` or a full dotted suffix such as `acme.com`. Each label is limited to 63 characters, and every full suffix and generated hostname is limited to 253 characters. `PORTLESS_SUFFIX` accepts comma-separated values, and repeatable `--tld` remains a compatibility alias:
...
The proxy auto-syncs `/etc/hosts` for route hostnames, so `.test`, `.acme.com`, and other dev hostnames resolve on your machine. Explicit lists are preserved in LAN mode with `.local` appended; mDNS publishes only the `.local` routes.
