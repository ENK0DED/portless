...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`--suffix <suffix>`</td>
      <td>Use a custom domain suffix instead of `.localhost` (e.g. `test`, `acme.com`). `--tld` still works as a compatibility alias. Hostnames still sync to `/etc/hosts` by default.</td>
    </tr>
    <tr>
      <td>`--cert <path>`</td>
