...
NetBird sharing requires the NetBird CLI to be connected, with Peer Expose enabled and allowed for the active peer. NetBird public URLs should be restricted with `--netbird-password`, `--netbird-pin`, or `--netbird-groups` when exposing production-like services.
