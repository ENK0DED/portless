...
- **Node.js 24**: The published package now requires Node.js 24 or newer, and fork repository development uses Bun
...
- **Project dev dependency install**: portless can now be installed as a project dev dependency (`npm install -D @enk0ded/portless`) in addition to the global install. The `npx`/`dlx` guard now only blocks one-off downloads, not locally installed packages
...
- **homepage**: Point npm homepage to https://portless.sh
