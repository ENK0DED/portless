...
describe("buildServiceSpec", () => {
  it("builds a macOS LaunchDaemon for the HTTPS proxy", () => {
    const spec = buildServiceSpec({
      ...
      entryScript: "/usr/local/lib/node_modules/@enk0ded/portless/dist/cli.js",
    ...
    expect(spec.programArguments).toEqual([
      "/usr/local/bin/node",
      "/usr/local/lib/node_modules/@enk0ded/portless/dist/cli.js",
      "proxy",
      "start",
      "--foreground",
      "--port",
  ...
  it("builds a Linux systemd unit for the HTTPS proxy", () => {
    ...
    expect(spec.execStart).toEqual([
      "/usr/bin/node",
      "/usr/lib/node_modules/@enk0ded/portless/dist/cli.js",
      "proxy",
      "start",
      "--foreground",
      "--port",
    ...
    expect(spec.unit).toContain(
      'ExecStart="/usr/bin/node" "/usr/lib/node_modules/@enk0ded/portless/dist/cli.js" "proxy" "start" "--foreground" "--port" "443" "--https" "--skip-trust"'
    );
  ...
  it("persists no-TLS, custom suffix, and custom state in a Linux service", () => {
    const spec = buildServiceSpec({
      platform: "linux",
      nodePath: "/usr/bin/node",
      entryScript: "/usr/lib/node_modules/@enk0ded/portless/dist/cli.js",
...
describe("handleService", () => {
  ...
  it("does not validate proxy env while uninstalling", async () => {
    ...
    const originalSuffix = process.env.PORTLESS_SUFFIX;
    process.env.PORTLESS_PORT = "abc";
    process.env.PORTLESS_SUFFIX = "bad_name";
    const runner = vi.fn((_: string, _args: string[]) => ({
      status: 0,
      stdout: "",
      stderr: "",
    ...
    try {
      ...
      if (originalSuffix === undefined) {
        ...
        process.env.PORTLESS_SUFFIX = originalSuffix;
      }
    }
