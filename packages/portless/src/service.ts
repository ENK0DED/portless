...
function getConfiguredServiceSuffix(
  env: NodeJS.ProcessEnv | Record<string, string>
): { value: string; source: typeof SUFFIX_ENV | typeof LEGACY_TLD_ENV } | null {
  const suffix = env[SUFFIX_ENV]?.trim().toLowerCase();
  if (suffix) {
    return { value: suffix, source: SUFFIX_ENV };
  }

  const legacy = env[LEGACY_TLD_ENV]?.trim().toLowerCase();
  if (legacy) {
