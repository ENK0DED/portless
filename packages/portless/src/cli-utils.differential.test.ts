...
const SCRIPTS: Record<string, string> = {
  ...
  backtickStart: String.fromCharCode(96) + "printf vite" + String.fromCharCode(96) + " dev",
  subshell: "(" + FRAMEWORK + ")",
...
const SHIMMED_BINARIES = ["vite", "vitepress", "rsbuild", "php", "expo", "wrangler", "next"];
...
const runners = isWindows
  ? []
  : (requestedRunners ?? ["bun", "npm", "pnpm", "yarn"].filter(hasBinary)).filter(hasBinary);
...
describe("package script forwarding against the shell", () => {
  ...
  it("identifies Expo even in script shapes it declines to modify", () => {
    const expoShapes = Object.keys(SCRIPTS).filter((name) => name.startsWith("expo"));
    const resolved = Object.fromEntries(
      expoShapes.map((name) => [name, resolveFrameworkBasename(["bun", "run", name], dir)])
    );
  ...
  it.skipIf(runners.length > 0)("reports when the shell sweep cannot run", () => {
    expect(isWindows || !hasBinary("bun")).toBe(true);
  });

  for (const runner of runners) {
    it(
      "never appends flags the " + runner + " child does not receive",
      () => {
        ...
        for (const [name, script] of Object.entries(SCRIPTS)) {
          ...
          const result = runScript(runner, name, injected);
          if (result.delivered === null) {
            unmeasured.push(name + ": " + JSON.stringify(script) + " (exit " + result.status + ")");
            continue;
          }
          const missing = injected.filter((flag) => !result.delivered!.includes(flag));
          if (missing.length > 0) {
            violations.push(
              name +
                ": " +
